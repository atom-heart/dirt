...
splits = [
    ...
    Split(track=tracks[3], weather=weather[1], order=2, last_in_stage=True),
...
times = [
    ...
    Time(player=players[4], split=splits[0]),
    Time(player=players[4], split=splits[1]),
    Time(player=players[4], split=splits[2]),
    Time(player=players[4], split=splits[3])
]
...
event_players[3].player = players[4]

event.event_players.extend(event_players)

# Create stages
stages = [
    Stage(order=1, event=event, country=countries[0], splits=splits[:2], last_in_event=False),
